    ...
    steps {
        ...
        script {
            ...
            scriptContent = """
                ...
                [ %disable_rngsse2% == no ] && module load rngsse2
                [ %disable_testu01% == no ] && module load testu01
                [ %stdlibcpp% == libc++ ] && module load libc++
                module load charm/%compiler%-%stdlibcpp% h5part/%compiler%-%stdlibcpp% trilinos/%compiler%-%stdlibcpp%/%mathlib%
                module load pugixml pegtl pstreams boost-1.61.0-gcc-4.8.5-q2hywin hypre-2.10.1-gcc-4.8.5-beuxbxv gmsh-2.12.0-gcc-4.8.5-p3vpjfb hdf5-1.10.0-patch1-gcc-4.8.5-mmtlfty netcdf-4.4.1-gcc-4.8.5-5xen4a5 random123 tut cartesian_product numdiff
                module list
                rm -rf build && mkdir build && cd build
                cmake -DCMAKE_CXX_COMPILER=mpicxx -DCMAKE_C_COMPILER=mpicc -DCMAKE_BUILD_TYPE=%buildtype% -DSTDLIBCPP=%stdlibcpp% -DCMAKE_DISABLE_FIND_PACKAGE_RNGSSE2=%disable_rngsse2% -DCMAKE_DISABLE_FIND_PACKAGE_TestU01=%disable_testu01% -DCMAKE_CXX_FLAGS=-Werror ../src
                #make -j16
            """.trimIndent()
        }
        script {
